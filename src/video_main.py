...
if K.image_dim_ordering() == 'th':
    input_shape = (1, time_length, feature_length, feature_length)
else:
    input_shape = (time_length, feature_length, feature_length, 1)
            ...
            for j in range(j0, len(align_list)):
                ...
                for k in range(k0, len(align)):
                    ...
                    if count >=size:
                        ...
                        return np.array(data_video), np.array(data_label), cookies
                k0 = 0
            j0 = 0
        i0 = start
...
#        input_length = np.array([np.ceil(k.shape[0]/4.0) for k in trainX])
        trainX = sequence.pad_sequences(np.array(trainX), maxlen=time_length, padding='post', dtype=float16).reshape(-1,time_length,feature_length,feature_length,1)
...
def get_test( size =100, cookies =[30,0,0]):
    while 1:
        data_audio, data_label, cookies = get_video_label1( size, cookies, 'test')
        print(cookies)
...
def decode_batch(test_func, word_batch, input_length):
    ret = []
    for i in range(word_batch.shape[0]//100):
        out = test_func([word_batch[i*100:(i+1)*100]])[0]
        #print ('out shape\n\n',out.shape)
...
inner = Conv3D(conv_num_filters, filter_size, border_mode='same',
                      activation=act, init='he_normal', name='conv1')(input_data)
inner = MaxPooling3D(pool_size=(1, pool_size, pool_size), name='max1')(inner)
# inner = Dropout(0.4)(inner)
inner = Conv3D(conv_num_filters, filter_size, border_mode='same',
                      activation=act, init='he_normal', name='conv2')(inner)
inner = MaxPooling3D(pool_size=(1, pool_size, pool_size), name='max2')(inner)
# inner = Dropout(0.4)(inner)

# inner = MaxPooling2D(pool_size=(pool_size, pool_size), name='max3')(inner)

conv_to_rnn_dims = (time_length , ((feature_length // (pool_size ** 2))**2) * conv_num_filters)
...
viz_cb = VizCallback(test_func, get_test(size=1000, cookies=[1,0,0]))
